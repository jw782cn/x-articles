# [2026-04-13] Cloudflare Sandboxes 正式 GA

**原文链接**：[Agents have their own computers with Sandboxes GA](https://blog.cloudflare.com/sandbox-ga/) by **Cloudflare****推文**：[@whoiskatrin](https://x.com/whoiskatrin/status/2043689502510580118) | 4.5万阅读 | 685点赞

当我们去年6月推出 [Cloudflare Sandboxes](https://github.com/cloudflare/sandbox-sdk) 时，前提很简单：[AI agent](https://www.cloudflare.com/learning/ai/what-is-agentic-ai/) 需要开发和运行代码，而它们需要在安全的地方做这件事。

如果 agent 像开发者一样工作，这意味着克隆仓库、用多种语言构建代码、运行开发服务器等等。要有效地做这些事，它们通常需要一台完整的计算机（如果不需要，也可以[选择更轻量的方案](https://blog.cloudflare.com/dynamic-workers/)）。

许多开发者正在用 VM 或现有容器方案拼凑解决方案，但有很多难题要解决：

我们花时间解决了这些问题，这样你就不用自己操心了。自初始发布以来，我们让 Sandboxes 成为更好的大规模运行 agent 的平台。我们与 Figma 等初始合作伙伴合作，他们在容器中运行 agent（[Figma Make](https://www.figma.com/make/)）：
...
Cloudflare Sandbox 是由 [Cloudflare Containers](https://blog.cloudflare.com/containers-are-available-in-public-beta-for-simple-global-and-programmable/) 驱动的持久、隔离环境。你通过名称请求一个 sandbox。如果它正在运行，你就获取它。如果没有运行，它会启动。空闲时自动休眠，收到请求时唤醒。你可以使用 exec、gitClone、writeFile 等[方法](https://developers.cloudflare.com/sandbox/api/)轻松地以编程方式与 sandbox 交互。
...
关于其工作原理的深入探讨——包括身份感知凭证注入、动态修改规则，以及与 [Workers bindings](https://developers.cloudflare.com/workers/runtime-apis/bindings/) 的集成——请阅读我们最近关于 [Sandbox 认证](https://blog.cloudflare.com/sandbox-auth)的博客文章。
...
2月，我们发布了 PTY 支持。这是 Sandbox 中的伪终端会话，通过 WebSocket 代理，兼容 [xterm.js](https://xtermjs.org/)。
...
当 agent 能够构建东西并立即展示给用户时，它们会更加有用。Sandboxes 支持后台进程、就绪检查和[预览 URL](https://developers.cloudflare.com/sandbox/concepts/preview-urls/)。这让 agent 可以启动开发服务器并在不离开对话的情况下分享实时链接。
...
我们在3月发布了 sandbox.watch()。它返回一个 SSE 流，底层基于原生 [inotify](https://man7.org/linux/man-pages/man7/inotify.7.html)——Linux 用于文件系统事件的内核机制。
...
快照存储在你账户中的 [R2](https://developers.cloudflare.com/r2/) 上，提供持久性和位置无关性。R2 的[分层缓存](https://developers.cloudflare.com/cache/how-to/tiered-cache/)系统允许在 Region: Earth 的任何地方快速恢复。

在未来的版本中，内存中的活跃状态也将被捕获，允许运行中的进程从上次中断的地方精确恢复。终端和编辑器将以关闭时的精确状态重新打开。

如果你有兴趣在快照正式上线之前恢复会话状态，可以使用现有的[备份和恢复](https://developers.cloudflare.com/sandbox/guides/backup-restore/)方法。这些方法也通过 R2 持久化和恢复目录，但性能不如真正的 VM 级快照。不过，与简单地重新创建会话状态相比，仍然可以带来显著的速度提升。
...
自初始发布以来，我们一直在稳步增加容量。标准定价方案的用户现在可以运行 15,000 个并发 lite 实例、6,000 个 basic 实例，以及超过 1,000 个更大的实例。想运行更多？[联系我们](https://forms.gle/3vvDvXPECjy6F8v56)！

我们还改变了定价模式，使大规模运行更具成本效益。Sandboxes 现在[只对实际使用的 CPU 周期收费](https://developers.cloudflare.com/changelog/post/2025-11-21-new-cpu-pricing/)。这意味着你不会在 agent 等待 LLM 响应时为空闲 CPU 付费。
